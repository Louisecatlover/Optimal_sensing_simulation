...
from optimal_sensing_sim.msg import output_uavs_pose
from optimal_sensing_sim.msg import output_uavs_controller
from optimal_sensing_sim.msg import output_ukf
from optimal_sensing_sim.msg import output_FOV
from optimal_sensing_sim.msg import output_tracking_distance
from optimal_sensing_sim.msg import output_inter_uavs_distance
from optimal_sensing_sim.msg import output_occlusion
from std_msgs.msg import Float64MultiArray

fx,fy,lx,ly = 381,381,640,480
target_length,target_width,target_height = 4.6,2.5,1.5
...
camera_cmd = TwistStamped()
ranging_cmd = TwistStamped()
...
def CBF():
	...
	theta_occ_cr = acos(np.dot(r_rc,r_qc)/r_rc_norm/r_qc_norm)
	theta_occ_cb = acos(np.dot(r_bc,r_qc)/r_bc_norm/r_qc_norm)
	theta_occ_rc = acos(np.dot(r_cr,r_qr)/r_cr_norm/r_qr_norm)
	theta_occ_rb = acos(np.dot(r_br,r_qr)/r_br_norm/r_qr_norm)
	theta_occ_bc = acos(np.dot(r_cb,r_qb)/r_cb_norm/r_qb_norm)
	theta_occ_br = acos(np.dot(r_rb,r_qb)/r_rb_norm/r_qb_norm)
	occlusion.theta_occ_cr.data = (180*theta_occ_cr)/pi
	occlusion.theta_occ_cb.data = (180*theta_occ_cb)/pi
	occlusion.theta_occ_rc.data = (180*theta_occ_rc)/pi
	occlusion.theta_occ_rb.data = (180*theta_occ_rb)/pi
	occlusion.theta_occ_bc.data = (180*theta_occ_bc)/pi
	occlusion.theta_occ_br.data = (180*theta_occ_br)/pi
...
def	qpsolver():
	...
	camera_cmd.twist.linear.z = optimal[2]
	ranging_cmd.twist.linear.x = optimal[3]
	ranging_cmd.twist.linear.y = optimal[4]
	ranging_cmd.twist.linear.z = optimal[5]
	bearing_cmd.twist.linear.x = optimal[6]
	bearing_cmd.twist.linear.y = optimal[7]
	bearing_cmd.twist.linear.z = optimal[8]
	camera_cmd.twist.angular.z = optimal[9]
...
if __name__ == '__main__':
	try:
		...
		px4_vel_pub1 = rospy.Publisher('/uav1/mavros/setpoint_velocity/cmd_vel', TwistStamped, queue_size=1)
		px4_vel_pub2 = rospy.Publisher('/uav2/mavros/setpoint_velocity/cmd_vel', TwistStamped, queue_size=1)
		px4_vel_pub3 = rospy.Publisher('/uav3/mavros/setpoint_velocity/cmd_vel', TwistStamped, queue_size=1)
		...
		while not rospy.is_shutdown():			
			...
			if uavs_pose_flag & uavs_controller_flag:
				...
				if optimal_sensing_mode:
					px4_vel_pub1.publish(camera_cmd)
					px4_vel_pub2.publish(ranging_cmd)
					px4_vel_pub3.publish(bearing_cmd)
			rate.sleep()
